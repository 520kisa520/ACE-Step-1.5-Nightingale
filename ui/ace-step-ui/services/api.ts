...
// Import Song type from types.ts for consistency
import { Song as InternalSong } from '../types';

// Re-export Song type for backward compatibility
export type Song = InternalSong;
...
// Songs API - Import Song type from types.ts for consistency
// API Song interface (raw from backend)
interface ApiSong {
  id: string;
  title: string;
  lyrics: string;
...
// Transform API song to internal Song type
function transformApiSong(apiSong: ApiSong): InternalSong {
  const rawUrl = apiSong.audio_url || apiSong.audioUrl;
  const resolvedUrl = getAudioUrl(rawUrl, apiSong.id);
  return {
    id: apiSong.id,
    title: apiSong.title,
    lyrics: apiSong.lyrics,
    style: apiSong.style,
    coverUrl: apiSong.cover_url || `https://picsum.photos/seed/${apiSong.id}/400/400`,
    duration: apiSong.duration && apiSong.duration > 0 
      ? `${Math.floor(apiSong.duration / 60)}:${String(Math.floor(apiSong.duration % 60)).padStart(2, '0')}` 
      : '0:00',
    createdAt: new Date(apiSong.created_at),
    tags: apiSong.tags || [],
    audioUrl: resolvedUrl,
    isPublic: apiSong.is_public,
    likeCount: apiSong.like_count || 0,
    viewCount: apiSong.view_count || 0,
    userId: apiSong.user_id,
    creator: apiSong.creator,
    ditModel: apiSong.ditModel,
    generationParams: (() => {
      try {
        if (!apiSong.generation_params) return undefined;
        return typeof apiSong.generation_params === 'string' 
          ? JSON.parse(apiSong.generation_params) 
          : apiSong.generation_params;
      } catch {
        return undefined;
      }
    })(),
...
function transformSongs(apiSongs: ApiSong[]): InternalSong[] {
  return apiSongs.map(transformApiSong);
}

export const songsApi = {
  getMySongs: async (token: string): Promise<{ songs: InternalSong[] }> => {
    const result = await api('/api/songs', { token }) as { songs: ApiSong[] };
    return { songs: transformSongs(result.songs) };
  },

  getPublicSongs: async (limit = 20, offset = 0): Promise<{ songs: InternalSong[] }> => {
    const result = await api(`/api/songs/public?limit=${limit}&offset=${offset}`) as { songs: ApiSong[] };
    return { songs: transformSongs(result.songs) };
  },

  getFeaturedSongs: async (): Promise<{ songs: InternalSong[] }> => {
    const result = await api('/api/songs/public/featured') as { songs: ApiSong[] };
    return { songs: transformSongs(result.songs) };
  },

  getSong: async (id: string, token?: string | null): Promise<{ song: InternalSong }> => {
    const result = await api(`/api/songs/${id}`, { token: token || undefined }) as { song: ApiSong };
    return { song: transformApiSong(result.song) };
  },

  getFullSong: async (id: string, token?: string | null): Promise<{ song: InternalSong, comments: any[] }> => {
    const result = await api(`/api/songs/${id}/full`, { token: token || undefined }) as { song: ApiSong, comments: any[] };
    return { ...result, song: transformApiSong(result.song) };
  },
