...
export const Player: React.FC<PlayerProps> = ({
    ...
    if (!currentSong) {
        return (
            <div className="h-20 lg:h-24 bg-white/80 dark:bg-slate-950/90 backdrop-blur-2xl border-t border-white/50 dark:border-white/10 flex items-center justify-center z-50 transition-colors duration-300 shadow-[0_-18px_40px_-18px_rgba(15,23,42,0.18)] dark:shadow-[0_-10px_40px_-20px_rgba(0,212,255,0.18)]">
                <button
                    onClick={() => onPlayFirst?.()}
                ...
                <div className="fixed inset-0 z-50 bg-gradient-to-b from-zinc-100 to-zinc-50 dark:from-zinc-900 dark:to-black flex flex-col safe-area-inset-top safe-area-inset-bottom transition-colors duration-300">
                    ...
                    <div className="flex items-center justify-center gap-8 py-4">
                        ...
                        <button
                            ...
                            className="w-16 h-16 rounded-full bg-gradient-to-br from-healing-primary to-healing-primary-dark dark:from-healing-primary-light dark:to-healing-primary text-white flex items-center justify-center shadow-xl shadow-healing-primary/20 tap-highlight-none hover:scale-105 transition-transform duration-200"
                        >
    ...
    return (
        <div className="h-20 lg:h-24 bg-white/85 dark:bg-healing-bg-dark/90 backdrop-blur-2xl border-t border-healing-primary/20 dark:border-healing-primary/30 flex flex-col z-50 transition-colors duration-300 shadow-healing-strong">

            {/* Progress Bar */}
            <div
                ref={progressBarRef}
                className="w-full h-1 lg:h-1.5 bg-gradient-to-r from-zinc-200 via-zinc-100 to-zinc-200 dark:from-healing-bg-card dark:via-healing-bg-hover dark:to-healing-bg-card cursor-pointer group relative"
                onClick={(e) => handleSeekInteraction(e, progressBarRef)}
            >
                <div
                    className="h-full bg-healing-primary dark:bg-healing-primary-light relative group-hover:bg-healing-primary-dark dark:group-hover:bg-healing-primary transition-colors"
                    style={{ width: `${progressPercent}%` }}
                >
                    <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 bg-healing-primary dark:bg-healing-primary-light group-hover:bg-healing-primary-dark dark:group-hover:bg-healing-primary rounded-full shadow-lg -mr-2 opacity-0 group-hover:opacity-100 transition-opacity" />
                </div>
            ...
            <div className="flex-1 flex items-center justify-between px-2 sm:px-4 lg:px-6 gap-2 sm:gap-4">
                ...
                <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1 max-w-[30%] lg:max-w-[33%]">
                    <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl bg-zinc-200 dark:bg-zinc-800 overflow-hidden shadow-sm ring-1 ring-black/5 dark:ring-white/10 flex-shrink-0">
                        {currentSong.coverUrl ? (
                            <img src={currentSong.coverUrl} className="w-full h-full object-cover" alt="cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                        ) : null}
                        {!currentSong.coverUrl && <AlbumCover seed={currentSong.id || currentSong.title} size="full" className="w-full h-full" />}
                    ...
                    <button
                        onClick={onToggleLike}
                        className={`ml-1 sm:ml-2 transition-colors flex-shrink-0 hidden sm:block ${isLiked ? 'text-healing-primary dark:text-healing-primary-light' : 'text-zinc-400 hover:text-healing-primary dark:hover:text-healing-primary-light'}`}
                    >
                ...
                <div className="flex flex-col items-center justify-center flex-shrink-0">
                    <div className="flex items-center gap-2 sm:gap-4 lg:gap-6">
                        <button
                            onClick={onToggleShuffle}
                            className={`transition-colors hidden sm:block ${isShuffle ? 'text-healing-primary dark:text-healing-primary-light' : 'text-zinc-400 hover:text-healing-primary dark:hover:text-healing-primary-light'}`}
                        >
                            <Shuffle size={16} />
                        </button>
                        <button
                            onClick={onPrevious}
                            className="text-zinc-700 dark:text-healing-text-secondary hover:text-healing-primary dark:hover:text-healing-primary-light transition-colors"
                        >
                            <SkipBack size={18} className="sm:w-[22px] sm:h-[22px]" fill="currentColor" />
                        </button>
                        <button
                            onClick={onTogglePlay}
                            className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-healing-primary to-healing-primary-dark dark:from-healing-primary-light dark:to-healing-primary text-white flex items-center justify-center hover:scale-105 transition-transform shadow-lg shadow-healing-primary/20"
                        >
                        ...
                        <button
                            onClick={onToggleRepeat}
                            className={`transition-colors hidden sm:block ${repeatMode !== 'none' ? 'text-healing-primary dark:text-healing-primary-light' : 'text-zinc-400 hover:text-healing-primary dark:hover:text-healing-primary-light'} relative`}
                        >
