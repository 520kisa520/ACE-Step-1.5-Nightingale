      ...
      <nav className="flex-1 flex flex-col gap-2 w-full px-3">
        ...
        <div className="mt-auto flex flex-col gap-2">
          ...
          <button
            ...
            className={`
              w-full rounded-2xl flex items-center gap-3 transition-all duration-300 text-zinc-500 dark:text-zinc-400 hover:text-black dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/5
              ${isOpen ? 'px-4 py-3 justify-start' : 'aspect-square justify-center'}
            `}
          ...
          {user ? (
            <>
              ...
              >
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold border border-white/20 overflow-hidden flex-shrink-0 ${
                  HEALING_MODE ? 'bg-gradient-to-br from-healing-primary to-healing-primary-dark' : 'bg-gradient-to-br from-pink-500 to-purple-600'
                }`}>
                  {user.avatar_url ? (
              ...
              <button
                onClick={onLogout}
                className={`
                  w-full rounded-2xl flex items-center gap-3 transition-all duration-300 text-zinc-500 hover:text-red-500 hover:bg-red-500/10
                  ${isOpen ? 'px-4 py-3 justify-start' : 'aspect-square justify-center'}
                `}
                title={t('signOut')}
              >
            ...
            <button
              onClick={onLogin}
              className={`
                w-full rounded-2xl flex items-center gap-3 transition-all duration-300 text-zinc-500 dark:text-zinc-400 hover:text-pink-500 hover:bg-zinc-100 dark:hover:bg-white/5
                ${isOpen ? 'px-4 py-3 justify-start' : 'aspect-square justify-center'}
              `}
              title={t('signIn')}
            >
...
const NavItem: React.FC<NavItemProps> = ({ icon, label, active, onClick, isExpanded }) => (
  <button
    onClick={onClick}
    className={`
      w-full rounded-2xl flex items-center gap-3 transition-all duration-300 group relative overflow-hidden
      ${isExpanded ? 'px-4 py-3 justify-start' : 'aspect-square justify-center'}
      ${active
        ? HEALING_MODE
          ? 'bg-healing-primary/20 text-healing-primary shadow-lg shadow-healing-primary/20'
          : 'bg-pink-500/20 text-pink-500 shadow-lg shadow-pink-500/20'
    ...
    {active && (
      <div
        className={`absolute left-0 top-1/2 -translate-y-1/2 h-10 w-1.5 rounded-r-full ${
          HEALING_MODE ? 'bg-healing-primary' : 'bg-pink-500'
        }`}
      />
    )}
