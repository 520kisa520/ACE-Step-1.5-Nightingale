...
import {
  ...
  Cpu, Wand2, Settings, RefreshCw, Sliders,
} from 'lucide-react';
...
export const TrainingPanel: React.FC = () => {
  ...
  // Training state - Healing mode optimized defaults
  ...
  const [collapsedSections, setCollapsedSections] = useState<Set<string>>(new Set(['loraSettings', 'trainingParameters']));
          ...
          <>
            ...
            <Section title={
              <button 
                onClick={() => setCollapsedSections(prev => {
                  ...
                  if (next.has('loraSettings')) next.delete('loraSettings');
                  else next.add('loraSettings');
                  return next;
                })}
                className="flex items-center gap-1.5 w-full text-left"
              >
                <Settings size={12} />
                <span>{t('loraSettings')}</span>
                <ChevronRight size={12} className={`ml-auto transition-transform ${collapsedSections.has('loraSettings') ? '' : 'rotate-90'}`} />
              </button>
            }>
              {!collapsedSections.has('loraSettings') && (
            ...
            <Section title={
              <button 
                onClick={() => setCollapsedSections(prev => {
                  const next = new Set(prev);
                  if (next.has('trainingParameters')) next.delete('trainingParameters');
                  else next.add('trainingParameters');
                  return next;
                })}
                className="flex items-center gap-1.5 w-full text-left"
              >
                <Sliders size={12} />
                <span>{t('trainingParameters')}</span>
                <ChevronRight size={12} className={`ml-auto transition-transform ${collapsedSections.has('trainingParameters') ? '' : 'rotate-90'}`} />
              </button>
            }>
              {!collapsedSections.has('trainingParameters') && (
                <div className="space-y-2 mt-2 animate-slide-up">
                  <FieldRow label={t('learningRate')}>
                    <input type="number" value={trainingParams.learningRate} onChange={e => setTrainingParams(p => ({ ...p, learningRate: parseFloat(e.target.value) || 0.0002 }))} step={0.0001} className="w-28 bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-pink-500/50" />
                  </FieldRow>
