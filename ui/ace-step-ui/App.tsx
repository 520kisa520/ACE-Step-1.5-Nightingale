...
function AppContent() {
  ...
  // Theme State - extended to support healing/dawn themes
  const [theme, setTheme] = useState<'dark' | 'light' | 'healing' | 'dawn'>(() => {
    const stored = localStorage.getItem('theme');
    if (stored === 'dark' || stored === 'light' || stored === 'healing' || stored === 'dawn') return stored as 'dark' | 'light' | 'healing' | 'dawn';
    if (HEALING_MODE) return 'dawn';
    return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'healing' : 'dawn';
  });
  ...
  useEffect(() => {
    ...
    document.documentElement.classList.remove('dark', 'healing', 'dawn');
    // Add the current theme class
    if (theme === 'dark') {
      document.documentElement.classList.add('dark');
    } else if (theme === 'healing') {
      document.documentElement.classList.add('dark', 'healing');
    } else if (theme === 'dawn') {
  ...
  const toggleTheme = () => {
    setTheme(prev => {
      // Cycle through themes: healing -> dawn -> light -> dark -> healing
      if (prev === 'healing') return 'dawn';
      if (prev === 'dawn') return 'light';
          ...
          <div className="flex h-full overflow-hidden relative w-full">
            ...
            <div className="md:hidden absolute top-4 right-4 z-50">
              <button
                ...
                className="bg-healing-primary text-white px-4 py-2 rounded-full shadow-lg shadow-healing-primary/20 border border-healing-primary/30 flex items-center gap-2 text-sm font-bold backdrop-blur-md"
              >
